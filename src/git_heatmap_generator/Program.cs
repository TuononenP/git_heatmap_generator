    ...
    static void Main(string[] args)
    {
        ...
        if (parsed.Layout == HeatmapLayout.Separate)
        {
            foreach (var year in parsed.Years.OrderByDescending(y => y))
            {
                ...
                string path = HeatmapRenderer.Generate(yearList, parsed.Emails, commitCounts, yearOutputPath, HeatmapLayout.Vertical, parsed.IncludePullRequests, parsed.Format, parsed.Theme, parsed.Mode, parsed.CustomColors, parsed.Use3DStyle, parsed.Use3DChart, parsed.Title, parsed.HideStats);
                Console.WriteLine($"Heatmap generated for {year}: {path}");
            }
        }
        else
        {
            string outputPath = HeatmapRenderer.Generate(parsed.Years, parsed.Emails, commitCounts, parsed.OutputFolder, parsed.Layout, parsed.IncludePullRequests, parsed.Format, parsed.Theme, parsed.Mode, parsed.CustomColors, parsed.Use3DStyle, parsed.Use3DChart, parsed.Title, parsed.HideStats);
